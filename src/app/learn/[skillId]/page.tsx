  ...
  return (
    <AppShell>
      <div className="mx-auto max-w-2xl">
        ...
        {/* skill header */}
        <div className="glass relative mb-8 overflow-hidden p-5">
          <div
        ...
        <div className="relative">
          <div className="absolute bottom-6 left-1/2 top-6 w-px -translate-x-1/2 bg-gradient-to-b from-white/15 via-white/8 to-white/15" />
                ...
                <div key={level.id}>
                  {isPremiumGate && (
                    <button
                      ...
                      className="relative z-10 mx-auto mb-4 flex w-full max-w-md items-center gap-3 rounded-2xl border border-amber-400/30 bg-gradient-to-r from-amber-500/10 to-yellow-500/10 p-4 text-left transition hover:border-amber-400/60"
                    >
      ...
      <Modal open={premiumOpen} onClose={() => setPremiumOpen(false)} title="👑 Unlock Premium Tiers">
        <p className="text-sm text-zinc-300">
          Tiers 7–10 of <span className="font-semibold text-zinc-100">{skill.title}</span> contain the Specialist,
        ...
        <div className="glass mt-4 flex items-center justify-between p-4">
          <span className="text-sm text-zinc-400">One-time unlock cost</span>
...
function LevelNode({
  ...
  const inner = (
    <div
      className={cn(
        "glass relative z-10 flex w-full max-w-md items-center gap-4 p-4 transition",
        unlocked && "hover:border-white/25 hover:bg-white/[0.07]",
        !unlocked && "opacity-60"
      )}
    >
      <div
        className={cn(
          "flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl font-mono text-lg font-black",
          completed ? "text-zinc-950" : "text-zinc-200",
          unlocked && !completed && "animate-pulse-glow"
        )}
        style={{
      ...
      <div className="min-w-0 flex-1">
        ...
        <div className="truncate text-sm font-semibold text-zinc-100">{level.title}</div>
        <div className="mt-0.5 flex items-center gap-2 text-[11px] text-zinc-500">
          <span className="font-mono text-yellow-400/90">+ↁ{level.coinReward}</span>
          <span className="font-mono text-violet-400/90">+{fmtNum(level.xpReward)} XP</span>
          {completed && (
            <span className="ml-auto flex items-center gap-0.5 font-mono text-emerald-400">
              <Star className="h-3 w-3 fill-emerald-400" /> {score}%
            </span>
          )}
        </div>
      </div>
      {unlocked && !completed && <Play className="h-4 w-4 shrink-0" style={{ color }} />}
    </div>
  );
